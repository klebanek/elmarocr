...
class ElmarApp {
  ...
  setupEventListeners() {
    ...
    document
      .getElementById("viewDocumentsBtn")
    ...
    document
      .getElementById("manageDatabaseBtn")
      .addEventListener("click", () => {
    ...
    document
      .getElementById("endWithoutSaveBtn")
      .addEventListener("click", () => {
        if (
          confirm(
            "Czy na pewno chcesz anulować? Wszystkie niezapisane produkty zostaną utracone.",
          )
        ) {
          this.showScreen("startScreen");
          this.currentDocument = null;
    ...
    document
      .getElementById("finishAndSaveBtn")
      .addEventListener("click", () => {
    ...
    document
      .getElementById("manualEntryScannerBtn")
      .addEventListener("click", () => {
        this.stopScanner();
        this.showScreen("manualEntryScreen");
      });
    ...
    document
      .getElementById("newProductDirectForm")
      .addEventListener("submit", (e) => {
        e.preventDefault();
    ...
    document
      .getElementById("cancelNewProductBtn")
      .addEventListener("click", () => {
  ...
  showScreen(screenId) {
    ...
    setTimeout(() => {
      const firstInput = newScreen.querySelector(
  ...
  async createNewDocument() {
    ...
    document.getElementById("documentDateDisplay").textContent = new Date(
      date,
  ...
  handleDirectNewProduct() {
    ...
    const productName = document
      .getElementById("directProductName")
      .value.trim();
    ...
    if (this.productDatabase[barcode]) {
      if (
        !confirm(
          `Kod ${barcode} już istnieje w bazie jako "${this.productDatabase[barcode]}". Czy chcesz go zastąpić?`,
        )
      ) {
        return;
      }
  ...
  searchProducts(searchTerm) {
    ...
    if (results.length === 0) {
      resultsContainer.innerHTML =
        '<p style="text-align: center; color: #666; padding: 1rem;">Nie znaleziono produktów</p>';
      return;
    }
        ...
        (result) => `
                    <div class="database-item">
                        <div class="database-item-info">
                            <div class="database-item-code">${result.barcode}</div>
    ...
    try {
      ...
      if (importedCount > 0) {
        ...
        this.showNotification(
          `Zaimportowano ${importedCount} produktów`,
          "success",
        );
      } else {
        this.hideLoadingOverlay();
        alert(
          "Nie znaleziono prawidłowych danych do importu. Sprawdź format pliku.",
        );
      }
    } catch (error) {
      this.hideLoadingOverlay();
  ...
  exportProductDatabase() {
    try {
      ...
      const sortedProducts = Object.entries(this.productDatabase).sort(
  ...
  processBarcodeDetectionEnhanced(data) {
    ...
    const quality =
      data.codeResult.decodedCodes.reduce(
        (sum, code) => sum + (code.error || 0),
        0,
      ) / data.codeResult.decodedCodes.length;
    ...
    if (value.length < 13) {
      ...
      if (this.productDatabase[value]) {
        this.showBarcodeStatus(
          `Produkt znaleziony: ${this.productDatabase[value]}`,
          "success",
        );
        input.style.borderColor = "var(--elmar-success)";
      } else {
        this.showBarcodeStatus(
          "Kod nie istnieje w bazie - zostanie dodany jako nowy produkt",
          "warning",
        );
        input.style.borderColor = "var(--elmar-warning)";
      }
  ...
  deleteDatabaseProduct(barcode) {
    ...
    if (
      confirm(
        `Czy na pewno chcesz usunąć produkt "${productName}" (${barcode}) z bazy?`,
      )
    ) {
  ...
  clearCustomProducts() {
    if (
      confirm(
        "Czy na pewno chcesz usunąć wszystkie produkty dodane ręcznie? Tej operacji nie można cofnąć.",
      )
    ) {
      try {
        const saved = localStorage.getItem("elmarCustomProducts");
        if (saved) {
          ...
          this.updateDatabaseStats();
          this.showNotification(
            "Usunięto wszystkie produkty ręczne",
            "success",
          );
        }
      } catch (error) {
  ...
  addProductToDocument(barcode, productName, quantity, expiryDate) {
    ...
    const existingIndex = this.currentDocument.products.findIndex(
      (p) => p.barcode === barcode && p.expiryDate === expiryDate,
    );

    if (existingIndex !== -1) {
      this.currentDocument.products[existingIndex].quantity += quantity;
      this.showNotification(
        `Zaktualizowano ilość produktu: ${productName}`,
        "success",
      );
    } else {
  ...
  updateProductsTable() {
    ...
    if (!this.currentDocument || this.currentDocument.products.length === 0) {
      ...
      tbody.innerHTML = this.currentDocument.products
        .map(
          (product, index) => `
                        <tr>
                            <td data-label="LP">${index + 1}</td>
                            <td data-label="Nazwa produktu">${this.escapeHtml(product.name)}</td>
                            <td data-label="Kod kreskowy">${product.barcode}</td>
  ...
  saveProductEdit() {
    ...
    this.currentDocument.products[this.currentEditIndex].expiryDate =
      expiryDate;
  ...
  updateDocumentsList() {
    ...
    if (sortedDocs.length === 0) {
      const message = this.showAllDocuments
        ? "Brak dokumentów"
        : "Brak dokumentów z dzisiejszej daty";
      container.innerHTML = `<div class="empty-state" style="text-align: center; padding: 2rem;">${message}</div>`;
    } else {
      container.innerHTML = sortedDocs
        .map(
          (doc) => `
  ...
  editDocument(docId) {
    ...
    if (this.currentDocument && this.currentDocument.products.length > 0) {
      if (
        !confirm(
          "Masz niezapisany dokument. Czy chcesz go porzucić i edytować wybrany dokument?",
        )
      ) {
        return;
      }
  ...
  showDocumentSelectionModal(docs, action) {
    const actionText =
      action === "generate" ? "Generuj PDF" : "Wyślij do biura";
    const modalHtml = `
    ...
    try {
      ...
      const generateBarcodeImage = (barcode) => {
        ...
        try {
          ...
          console.error(
            "Błąd generowania kodu kreskowego dla:",
            barcode,
            error,
          );
          return null;
      ...
      selectedDocs.forEach((document, docIndex) => {
        ...
        yPos += 7;
        doc.text(
          "Data: " + new Date(document.date).toLocaleDateString("pl-PL"),
          20,
          yPos,
        );
        yPos += 7;
        doc.text(
          "Utworzono: " + new Date(document.createdAt).toLocaleString("pl-PL"),
          20,
          yPos,
        );
        yPos += 7;
        doc.text(
          "Ostatnia modyfikacja: " + new Date().toLocaleString("pl-PL"),
          20,
          yPos,
        );
            ...
            if (data.section === "body" && data.column.index === 5) {
              ...
              if (barcodeImage) {
                try {
                  ...
                  console.error(
                    "Błąd dodawania obrazu kodu kreskowego:",
                    error,
                  );
                }
              }
        ...
        doc.text(
          "Laczna liczba pozycji: " + document.products.length,
          20,
          finalY + 7,
        );
        doc.text(
          "Laczna liczba sztuk: " +
            document.products.reduce((sum, p) => sum + p.quantity, 0),
          20,
          finalY + 14,
        );
      });
      ...
      const filename =
        "ELMAR_Dokumenty_" + new Date().toISOString().split("T")[0] + ".pdf";
      doc.save(filename);
  ...
  endWork() {
    const hasUnsavedWork =
      this.currentDocument && this.currentDocument.products.length > 0;
    let confirmMessage = "Czy na pewno chcesz zakończyć pracę?";

    if (hasUnsavedWork) {
      confirmMessage =
        "Masz niezapisany dokument! Czy na pewno chcesz zakończyć pracę? Niezapisane zmiany zostaną utracone.";
    }
  ...
  restoreCurrentDocument() {
    ...
    document.getElementById("workerName").value =
      this.currentDocument.worker || "";
    document.getElementById("documentDate").value =
      this.currentDocument.date || "";
    document.getElementById("notes").value = this.currentDocument.notes || "";

    document.getElementById("documentWorker").textContent =
      this.currentDocument.worker || "";
    document.getElementById("documentDateDisplay").textContent = this
      .currentDocument.date
      ? new Date(this.currentDocument.date).toLocaleDateString("pl-PL")
      : "";
  ...
  toggleAllDocuments() {
    ...
    if (subtitle) {
      subtitle.textContent = this.showAllDocuments
        ? "Wszystkie dokumenty"
...
Czy jesteś absolutnie pewien?`;
    ...
    if (confirm(confirmMessage)) {
      ...
      if (confirm("Ostatnie potwierdzenie - usunąć wszystkie dokumenty?")) {
        ...
        this.updateDocumentsList();
        this.showNotification(
          "Wszystkie dokumenty zostały usunięte",
          "success",
        );
      }
    }
                        ...
                        <div id="deleteDocumentsList" style="margin-bottom: var(--space-lg); max-height: 400px; overflow-y: auto;">
                            ${sortedDocs
                              .map((doc) => {
                                ...
                                const totalQuantity = doc.products.reduce(
...
Czy jesteś pewien?`;
    ...
    if (confirm(confirmMessage)) {
      ...
      this.updateDocumentsList();
      this.showNotification(
        `Usunięto ${selectedIds.length} dokumentów`,
        "success",
      );
    }
  }
    ...
    try {
      const exportData = {
        ...
        customProducts: JSON.parse(
          localStorage.getItem("elmarCustomProducts") || "{}",
        ),
        ...
        try {
          ...
          if (
            ...
            if (importData.customProducts) {
              localStorage.setItem(
                "elmarCustomProducts",
                JSON.stringify(importData.customProducts),
              );
            }
          ...
          console.error("Import error:", error);
          this.showNotification(
            "Błąd podczas importu: " + error.message,
            "error",
          );
        }
      };
                                ...
                                <tbody>
                                    ${sortedDocs
                                      .map((doc, index) => {
                                        const docPositions =
                                          doc.products.length;
                                        const docQuantity = doc.products.reduce(
                                          (sum, p) => sum + p.quantity,
                                          0,
                                        );
                                        return `
  ...
  exportSummaryToExcel(dateFrom, dateTo) {
    ...
    data.push([
      `Okres: ${new Date(dateFrom).toLocaleDateString("pl-PL")} - ${new Date(dateTo).toLocaleDateString("pl-PL")}`,
    ]);
